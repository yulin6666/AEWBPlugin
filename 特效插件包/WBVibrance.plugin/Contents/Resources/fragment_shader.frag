...
vec3 vibrance(vec3 inCol, float mVibrance) {
    vec3 outCol;
    
    float avg = dot(inCol, vec3(0.299, 0.587, 0.114));
    outCol = mix(vec3(avg), inCol, mVibrance);
    
    return outCol;
    //return clamp(outCol,0.0,1.0);
}

vec3 vibrance3(vec3 inCol, float mVibrance) {
    vec3 outCol;
    
    outCol = inCol;
    float average = (outCol.r + outCol.g + outCol.b) / 3.0;
    float mx = max(outCol.r, max(outCol.g, outCol.b));
    float amt = (mx - average) * (-mVibrance * 3.0);
    outCol.rgb = mix(outCol.rgb, vec3(mx), amt);
...
{
    vec3 outCol;
    //outCol = nlRGB2RGB(image.rgb);
    //if (vibrance <= 1.0) {
        //float avg = dot(image.rgb, vec3(0.299, 0.587, 0.114));
        //vibrance += 1.0f;
    
        //float avg = (image.x + image.y + image.z)/3.0;
        //outCol = mix(vec3(avg), image.rgb, vibrance);
        
        //float mx = max(image.r, max(image.g, image.b));
        //float amt = (mx - avg) * (-vibrance * 3.0);
        //image.rgb = mix(image.rgb, vec3(mx), amt);
    ...
    if(vibrance < 0.0) {
        ...
        //mediump float dampen_factor = 0.4;  // 0.7
        //mediump float scale_factor = 1.5;   // 2.0
        ...
        //float gray = dot(rgb1.rgb, vec3(0.299, 0.587, 0.114));
        ...
        //float gray_ = dot(image.rgb, vec3(0.299, 0.587, 0.114));
        
        outCol = clamp(image + (image - gray_) * boost,0.0,1.0);
        outCol = outCol + delta;
    //}
    //outCol.rgb = RGB2nlRGB(outCol);
    return clamp(outCol,0.0,1.0);
    //return outCol;
}
...
void main(void) {
    ...
    //img_adj = nlRGB2RGB(rgbaColor.rgb);
    
    /*if(uVibrance < 0.0f) {
        mVibrance = uVibrance * 0.5f;
    ...
    //rgbaColor.rgb = RGB2nlRGB(img_adj);
