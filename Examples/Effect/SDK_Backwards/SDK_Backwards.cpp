	...
	PF_LayerDef		*output )
{
	...
	if (out_data->dest_snd.fi.num_channels == in_data->src_snd.fi.num_channels) {
		...
		if ((in_data->src_snd.fi.format == PF_SIGNED_FLOAT) &&
			...
			while(out_dur_sampL--) {
				register	A_long iL = in_data->src_snd.fi.num_channels;

				while(iL--) {
					register PF_FpLong	accumF = *srcP;
			...
			while(out_dur_sampL--) {
				register	A_long iL = in_data->src_snd.fi.num_channels;
				while(iL--) {
					register PF_FpLong	accumF = AEFX_Type16toF(*srcP);
			...
			while(out_dur_sampL--){
				register	A_long iL = in_data->src_snd.fi.num_channels;
				while(iL--) {
					register PF_FpLong	accumF = AEFX_Type8toF(*srcP);
