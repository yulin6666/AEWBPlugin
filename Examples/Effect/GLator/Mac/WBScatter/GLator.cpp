...
namespace {
                  ...
                  float             random)
    {
        ...
        location = glGetUniformLocation(renderContext->mProgramObjSu, "width");
        glUniform1f(location, widthL);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "height");
        glUniform1f(location, heightL);
        
        location = glGetUniformLocation(renderContext->mProgramObjSu, "uAmount");
        glUniform1f(location, amount);
        
        location = glGetUniformLocation(renderContext->mProgramObjSu, "uType");
        glUniform1i(location, type);
        
        location = glGetUniformLocation(renderContext->mProgramObjSu, "uRandom");
        glUniform1f(location, random);
    ...
    PF_LayerDef        *output )
{
    ...
    PF_ADD_FLOAT_SLIDER(STR(StrID_Amount),
            0.f,
            +127.f,
            0.0f,
            +127.f,
    ...
    PF_ADD_POPUP(STR(StrID_Particle),
                 3,
                 1,
                 "both |"
                 "level |"
    ...
    PF_ADD_CHECKBOX(STR(StrID_Random),
                    STR(StrID_Random2),
                    0,
                    0,
                    3);
    ...
    PF_SmartRenderExtra        *extra)
{
    ...
    PF_ParamDef type_param;
    PF_ParamDef isRandom_param;
    ...
    AEFX_CLR_STRUCT(isRandom_param);
    ERR(PF_CHECKOUT_PARAM(in_data,
                          3,
        in_data->current_time,
        in_data->time_step,
    ...
    if (!err){
        mAmount = static_cast<float>(amount_param.u.fs_d.value);
        mType = static_cast<int>(type_param.u.td.x_value);
        isRandom = static_cast<bool>(isRandom_param.u.bd.value);
    }
    ...
    float lRandom = (float)random;
    float rRandom = (float)random * 0.1;

    float uRandom;

    if(isRandom) {
        uRandom = lRandom + rRandom;
    } else {
        uRandom = 1.0;
    }
    //**************************************************//
    ...
    if (!err){
        try
        {
            ...
            RenderGL(renderContext, widthL, heightL, inputFrameTexture, mAmount, mType, uRandom);
    ...
    const char* inHostVersion)
{
    ...
    result = PF_REGISTER_EFFECT(
        ...
        "WBTest", // Name
        "ADBE WBTest", // Match Name
        "ADBE WBTest", // Category
