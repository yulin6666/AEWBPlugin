...
namespace {
                  ...
                  float            saturation)
    {
        ...
        glUniformMatrix4fv(location, 1, GL_FALSE, (GLfloat*)&ModelviewProjection);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "uVibrance");
        glUniform1f(location, vibrance);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "uSaturation");
    ...
    PF_LayerDef        *output )
{
    ...
    PF_ADD_FLOAT_SLIDER(STR(StrID_Vibrance),
            -100.f,
            +100.f,
            -100.f,
            +100.f,
    ...
    PF_ADD_FLOAT_SLIDER(STR(StrID_Saturation),
            -100.f,
            +100.f,
            -100.f,
            +100.f,
    ...
    PF_SmartRenderExtra        *extra)
{
    ...
    PF_ParamDef vibrance_param;
    PF_ParamDef saturation_param;

    AEFX_CLR_STRUCT(vibrance_param);
    ERR(PF_CHECKOUT_PARAM(in_data,
                          1,
        in_data->current_time,
        in_data->time_step,
        in_data->time_scale,
        &vibrance_param));
    AEFX_CLR_STRUCT(saturation_param);
    ERR(PF_CHECKOUT_PARAM(in_data,
                          2,
        in_data->current_time,
        in_data->time_step,
    ...
    if (!err){
        mVibrance = static_cast<float>(vibrance_param.u.fs_d.value);
        mSaturation = static_cast<float>(saturation_param.u.fs_d.value);
    }
    mVibrance *= 0.01f;
...
//            RenderGL(renderContext, widthL, heightL, inputFrameTexture, sliderVal, multiplier16bit);
            RenderGL(renderContext, widthL, heightL, inputFrameTexture, mVibrance, mSaturation);
    ...
    ERR2(PF_CHECKIN_PARAM(in_data, &vibrance_param));
