...
namespace {
                  ...
                  PF_Boolean           inverse)
	{
		...
		glUniform1f(location, tileX);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "property_float_1");
        glUniform1f(location, tileY);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "property_float_2");
        glUniform1f(location, centerX);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "property_float_3");
        glUniform1f(location, centerY);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "property_float_4");
        glUniform1f(location, screenW);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "property_float_5");
        glUniform1f(location, screenH);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "property_float_6");
        glUniform1f(location, dirft);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "property_int_0");
        glUniform1i(location, inverse);
	...
	PF_LayerDef		*output )
{
	...
	PF_ADD_SLIDER(	STR(StrID_Name),
                    ...
                    GLATOR_SLIDER_MAX,
					1);
    PF_ADD_SLIDER(  STR(StrID_TileHeight),
                    GLATOR_SLIDER_MIN,
                    GLATOR_SLIDER_MAX,
                    GLATOR_SLIDER_MIN,
                    GLATOR_SLIDER_MAX,
    ...
    PF_ADD_ANGLE(STR(StrID_Drift),
                    0,
                    6);
    PF_ADD_CHECKBOX(STR(StrID_InverseGradient),
                    STR(StrID_InverseGradient),
                    FALSE,
                    7,
                    CHECKBOX_ID);
	...
	PF_SmartRenderExtra		*extra)
{
	...
	AEGP_SuiteHandler suites(in_data->pica_basicP);
    PF_ParamDef tileWidth_param;
    PF_ParamDef tileHeight_param;
    PF_ParamDef center_param;
    PF_ParamDef screenWidth_param;
    ...
    ERR(PF_CHECKOUT_PARAM(in_data,
        ...
        in_data->time_scale,
        &tileWidth_param));
    AEFX_CLR_STRUCT(tileHeight_param);
    ERR(PF_CHECKOUT_PARAM(in_data,
                          GLATOR_TileHeight,
        in_data->current_time,
        in_data->time_step,
	...
	ERR(PF_CHECKOUT_PARAM(in_data,
		...
		in_data->time_scale,
		&center_param));
    AEFX_CLR_STRUCT(screenWidth_param);
    ERR(PF_CHECKOUT_PARAM(in_data,
                          GLATOR_ScreenWidth,
        in_data->current_time,
        in_data->time_step,
        in_data->time_scale,
        &screenWidth_param));
    AEFX_CLR_STRUCT(screenHeight_param);
    ERR(PF_CHECKOUT_PARAM(in_data,
                          GLATOR_ScreenHeight,
        in_data->current_time,
        in_data->time_step,
	...
	if (!err){
        ...
        screenWidth = screenWidth_param.u.fd.value;
        screenHeight = screenHeight_param.u.fd.value;
        drift = (int(drift_param.u.ad.value / 65536.0) % 360) / 360.0 * 3.14;
...
//			RenderGL(renderContext, widthL, heightL, inputFrameTexture, sliderVal, multiplier16bit);
            RenderGL(renderContext, widthL, heightL, inputFrameTexture, multiplier16bit, tileWidth, tileHeight, centerX, centerY, screenWidth, screenHeight, drift, inverse);
	...
	const char* inHostVersion)
{
	...
	result = PF_REGISTER_EFFECT(
		...
		"WBImageWipe", // Name
		"ADBE WBImageWipe", // Match Name
        "ADBE WBImageWipe", // Category
