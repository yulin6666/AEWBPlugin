...
namespace {
    ...
    void RenderGL(const AESDK_OpenGL::AESDK_OpenGL_EffectRenderDataPtr &renderContext, A_long widthL, A_long heightL,
            gl::GLuint inputFrameTexture, int Hue, float multiplier16bit, float saturation, float lightness) {
        ...
        glUniformMatrix4fv(location, 1, GL_FALSE, (GLfloat *) &ModelviewProjection);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "Hue");
        glUniform1i(location, Hue);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "saturation");
        glUniform1f(location, saturation);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "lightness");
        glUniform1f(location, lightness);
...
ParamsSetup(
    ...
    AEFX_CLR_STRUCT(def);
    PF_ADD_ANGLE(STR(StrID_Name_Hue), 0, 1);

    AEFX_CLR_STRUCT(def);

    PF_ADD_FLOAT_SLIDER(STR(StrID_Name_Saturation),
            -100.f,
            +100.f,
            -100.f,
            +100.f,
    ...
    PF_ADD_FLOAT_SLIDER(STR(StrID_Name_Lightness),
            -100.f,
            +100.f,
            -100.f,
            +100.f,
...
SmartRender(PF_InData *in_data, PF_OutData *out_data, PF_SmartRenderExtra *extra) {
    ...
    float lightness = 0.01f;
    float saturation = 0.0f;
    signed int hue = 0;

    AEGP_SuiteHandler suites(in_data->pica_basicP);

    PF_ParamDef hue_param;
    PF_ParamDef saturationVal_param;
    PF_ParamDef gamma_param;

    AEFX_CLR_STRUCT(hue_param);
    ERR(PF_CHECKOUT_PARAM(in_data,
            1,
            in_data->current_time,
            in_data->time_step,
    ...
    AEFX_CLR_STRUCT(saturationVal_param);
    ERR(PF_CHECKOUT_PARAM(in_data,
            2,
            in_data->current_time,
            in_data->time_step,
    ...
    ERR(PF_CHECKOUT_PARAM(in_data,
            ...
            &gamma_param));
    if (!err) {
        lightness = static_cast<float>(gamma_param.u.fs_d.value);
        saturation = static_cast<float>(saturationVal_param.u.fs_d.value);
        hue = (hue_param.u.ad.value)/65536;
    }

    printf("\n SmartRender lightness = %f,saturation = %f,hue = %d", lightness, saturation, hue);
...
//			RenderGL(renderContext, widthL, heightL, inputFrameTexture, sliderVal, multiplier16bit);
            RenderGL(renderContext, widthL, heightL, inputFrameTexture, hue, multiplier16bit, saturation, lightness);
