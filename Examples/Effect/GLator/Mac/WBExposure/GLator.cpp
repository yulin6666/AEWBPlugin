...
/* suppliers and may be covered by U.S. and Forreign Patents,       */
...
namespace {
    ...
    std::string GetResourcesPath(PF_InData *in_data) {
        ...
        NSString *newStr = [[NSString alloc] initWithCharacters:
        pluginFolderPath
        length :
        length];
        std::string resourcePath([newStr
        UTF8String]);
            ...
            float &multiplier16bitOut)                    // <<
    {
        ...
        switch (format) {
            ...
            default: CHECK(PF_Err_BAD_CALLBACK_PARAM);
                break;
        }
    ...
    void RenderGL(const AESDK_OpenGL::AESDK_OpenGL_EffectRenderDataPtr &renderContext, A_long widthL, A_long heightL,
            gl::GLuint inputFrameTexture, float exposure, float multiplier16bit, float offset, float gammaOffset) {
        ...
        glUniformMatrix4fv(location, 1, GL_FALSE, (GLfloat *) &ModelviewProjection);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "exposure");
        glUniform1f(location, exposure);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "offset");
        glUniform1f(location, offset);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "gammaOffset");
        glUniform1f(location, gammaOffset);
...
ParamsSetup(
    ...
    PF_ADD_FLOAT_SLIDER(STR(StrID_Name_Exposure),
            static_cast<PF_FpShort>(-4.0f),
            4.0f,
            -4.0f,
            4.0f,
            0.01f,
    ...
    PF_ADD_FLOAT_SLIDER(STR(StrID_Name_Offset),
            static_cast<PF_FpShort>(-0.5f),
            0.5f,
            -0.5f,
            0.5f,
            0.01f,
    ...
    PF_ADD_FLOAT_SLIDER(STR(StrID_Name_Gamma),
            static_cast<PF_FpShort>(0.1f),
            10.0f,
            0.1f,
            10.0f,
            0.01f,
...
SmartRender(PF_InData *in_data, PF_OutData *out_data, PF_SmartRenderExtra *extra) {
    ...
    PF_ParamDef expVal_param;
    PF_ParamDef offsetVal_param;
    PF_ParamDef gamma_param;

    AEFX_CLR_STRUCT(expVal_param);
    ERR(PF_CHECKOUT_PARAM(in_data,
            1,
            in_data->current_time,
            in_data->time_step,
    ...
    AEFX_CLR_STRUCT(offsetVal_param);
    ERR(PF_CHECKOUT_PARAM(in_data,
            2,
            in_data->current_time,
            in_data->time_step,
    ...
    ERR(PF_CHECKOUT_PARAM(in_data,
            ...
            &gamma_param));
    if (!err) {
        gamma = static_cast<float>(gamma_param.u.fs_d.value);
        offset = static_cast<float>(offsetVal_param.u.fs_d.value);
        expValue = static_cast<float>(expVal_param.u.fs_d.value);
    }

    printf("\n SmartRender gamma = %f,offset = %f,exp = %f", gamma, offset, expValue);
...
//			RenderGL(renderContext, widthL, heightL, inputFrameTexture, sliderVal, multiplier16bit);
            RenderGL(renderContext, widthL, heightL, inputFrameTexture, expValue, multiplier16bit, offset, gamma);
