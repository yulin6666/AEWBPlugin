...
namespace {
                  ...
                  )
	{
		...
		glUniformMatrix4fv(location, 1, GL_FALSE, (GLfloat*)&ModelviewProjection);
		location = glGetUniformLocation(renderContext->mProgramObjSu, "density");
		glUniform1i(location, int(sliderVal));
		location = glGetUniformLocation(renderContext->mProgramObjSu, "multiplier16bit");
		glUniform1f(location, multiplier16bit);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "ColorPop");
        glUniform1i(location, colorIndex);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "palette_r");
        glUniform1i(location, red);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "palette_g");
        glUniform1i(location, green);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "palette_b");
        glUniform1i(location, blue);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "palette_a");
        glUniform1i(location, alpha);
        location = glGetUniformLocation(renderContext->mProgramObjSu, "keepluminosity");
        glUniform1i(location, keep);
	...
	PF_LayerDef		*output )
{
	...
	PF_ADD_SLIDER(	STR(StrID_Name),
					...
					GLATOR_SLIDER_DFLT,
					SLIDER_DISK_ID);
    PF_ADD_POPUP(STR(StrID_ColorPop),
                 3,
                 1,
    ...
    PF_ADD_CHECKBOX(STR(StrID_CheckBox),
                    STR(StrID_CheckBox),
                    1,
                    4,
                    4);
	...
	PF_SmartRenderExtra		*extra)
{
    ...
    PF_FpLong           colorIndex = 0;
    PF_ParamValue       keepLum = 0;
    PF_ParamValue       colorRed = 0;
    PF_ParamValue       colorGreen = 0;
    PF_ParamValue       colorBlue = 0;
    PF_ParamValue       colorAlpha = 0;

	AEGP_SuiteHandler suites(in_data->pica_basicP);

	PF_ParamDef slider_param;
    PF_ParamDef pop_para;
    PF_ParamDef Palette_param;
    ...
    ERR(PF_CHECKOUT_PARAM(in_data,
        ...
        in_data->time_scale,
        &pop_para));
    AEFX_CLR_STRUCT(checkBox_param);
    ERR(PF_CHECKOUT_PARAM(in_data,
                          3,
        in_data->current_time,
        in_data->time_step,
	...
	if (!err){
        ...
        colorIndex = pop_para.u.pd.value;
        keepLum = checkBox_param.u.bd.value;
        colorAlpha = Palette_param.u.cd.value.alpha;
        colorRed = Palette_param.u.cd.value.red;
        colorBlue = Palette_param.u.cd.value.blue;
        colorGreen = Palette_param.u.cd.value.green;
    }
	...
	if (!err){
		try
		{
            ...
            RenderGL(renderContext, widthL, heightL, inputFrameTexture, sliderVal, multiplier16bit, colorIndex, colorRed, colorGreen, colorBlue, colorAlpha,keepLum);
	...
	const char* inHostVersion)
{
	...
	result = PF_REGISTER_EFFECT(
		...
		"WBDirectionalBlur", // Name
		"ADBE WBDirectionalBlur", // Match Name
        "ADBE WBDirectionalBlur", // Category
